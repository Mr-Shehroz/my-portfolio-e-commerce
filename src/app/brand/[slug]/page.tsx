...
const BrandPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchBrandAndProducts = async () => {
      try {
        // Fetch brand info
        const brandQuery = `*[_type == "brand" && slug.current == $slug][0]{
          _id,
          title,
          "slug": slug.current,
          description,
        ...
        const brandData = await client.fetch(brandQuery, { slug });
        ...
        // Fetch products by brand reference
        const productQuery = `*[_type == "product" && brand._ref == $brandId] | order(_createdAt desc){
          _id,
          name,
          "slug": slug.current,
          price,
        ...
        const productData = await client.fetch(productQuery, { brandId: brandData._id });
  ...
  if (error || !brand) {
    return (
      <div className="bg-black text-white min-h-screen flex flex-col items-center justify-center px-4">
        ...
        <p className="text-gray-400 mb-6">{error || "The brand you're looking for doesn't exist."}</p>
  ...
  return (
    <div className="bg-black text-white relative min-h-screen">
      ...
      {/* Hero Banner */}
      <section className="relative py-28 md:py-36 w-full flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          {brand.image ? (
            <img
              src={urlFor(brand.image).width(1920).url()}
              alt={brand.title}
        ...
        <div className="text-center z-10 px-4">
          <div className="w-24 h-24 rounded-full bg-white/10 backdrop-blur-sm flex items-center justify-center mx-auto mb-6 border-2 border-red-600/30">
            {brand.image ? (
              <img
                src={urlFor(brand.image).width(100).height(100).url()}
                alt={brand.title}
                className="w-16 h-16 object-contain"
              />
            ) : (
              <span className="text-2xl font-bold text-white">{brand.title.charAt(0)}</span>
            )}
          </div>
          ...
          {products.length === 0 ? (
            <div className="text-center py-24">
              ...
              <p className="text-gray-400 text-xl font-medium">
                No products available from {brand.title} yet.
              </p>
            ...
            <>
              <div className="flex justify-between items-center mb-10">
                <h2 className="text-2xl md:text-3xl font-extrabold">
                  Products by <span className="text-red-600">{brand.title}</span>
                </h2>
