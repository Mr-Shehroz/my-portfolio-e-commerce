...
const NewArrivals: React.FC<Props> = ({ products }) => {
  ...
  return (
    <section className="py-20 bg-linear-to-br from-black via-gray-900 to-black relative overflow-hidden">
      {/* Subtle background pattern (same as Featured Products) */}
            ...
            {newArrivals.map((product) => (
              ...
              >
                <div className="relative h-64 overflow-hidden bg-gray-800">
                  ...
                  {/* Status Badge – only "new" but styled like before */}
                  {product.status && (
                    <div className="absolute top-4 right-4 bg-green-600 text-white px-3 py-1 rounded-full text-xs font-bold uppercase animate-pulse">
                      {product.status}
                    </div>
                  )}
                ...
                <div className="p-6">
                  {product.brand?.title && (
                    <p className="text-gray-400 text-sm mb-1 font-medium">{product.brand.title}</p>
                  )}
